...
//         //check if type is one-off but number_weeks and number_months are not null
...
//         }
//         //check if type is recurrent but number_weeks and number_months are null
...
//         }
//         $tenant_data = Tenant::where('slug', $slug)->first();
//         $spot_id = $validated['spot_id'];
//         $invoice_user = User::where('id', $validated['user_id'])->select('first_name', 'last_name')->first();
...
//         // ✅ EARLY CHECK FOR DUPLICATE RESERVED SPOTS
...
//         // Tax Calculation
//         $tax_data = [];
//         $taxes = TaxModel::where('tenant_id', $tenant->tenant_id)->get();
...
//             'tenant_id' => $tenant->tenant_id,
//         ], $spot_id);
//         $bookspot = BookSpot::where('id', $bookSpot->id)->first(); 
//         $bookspot->update([
//             'invoice_ref' => $invoiceResponse['invoice']['invoice_ref'],
//         ]); 
        
//         //update space payment
//         $spacePayment = SpacePaymentModel::where('payment_ref', $reference)->first();
//         if ($spacePayment) {
//             $spacePayment->update([
//                 'invoice_ref' => $invoiceResponse['invoice']['invoice_ref'],
//             ]);
...
// $expiryDate = Carbon::parse($bookSpot->expiry_day); // already Carbon, but ensures consistency

// $schedule = [];

// foreach ($chosenDays as $day) {
//     $weekday = strtolower($day['day']); // e.g., "thursday"
//     $startTime = Carbon::parse($day['start_time'])->format('H:i:s');
//     $endTime = Carbon::parse($day['end_time'])->format('H:i:s');

//     // Start from the first occurrence on/after the start date
//     $current = Carbon::parse($day['start_time'])->copy();

//     // Loop weekly until expiry
...
//         $current->addWeek(); // move to same weekday next week
//     }
// }
...
//             'user_invoice' => $invoice_user->first_name.' '.$invoice_user->last_name,
//             'tenant_name' => $tenant_data->company_name,
//             'book_data'=>$bookSpot,
//             'taxes'=> $tax_data,
//             'invoice_ref' => $invoiceResponse['invoice_ref'],
//             'schedule' => $schedule,
//         ];
...
// Mail::to($invoice_user->email)->send(new BookingInvoiceMail($data_invoice));
// //$pdf = PDF::loadView('emails.invoice', ['invoice' => $data_invoice]);
